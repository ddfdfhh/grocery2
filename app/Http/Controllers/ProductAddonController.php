...
class ProductAddonController extends Controller
{
    public function __construct()
    {
        ...
        $this->index_url=route('product_addons.index');
        $this->module='ProductAddon';
        $this->view_folder='product_addons';
        $this->storage_folder=$this->view_folder;
        $this->has_upload=1;
        $this->is_multiple_upload=0;
        $this->has_export=0;
        $this->pagination_count=100;
        $this->crud_title='Product Addon';
  ...
  public function sideColumnInputs($model=null)
    ...
    [
        ...
        $data['data'] = [
            ...
            'plural_lowercase' =>'product_addons',
            ...
            'storage_folder' => $this->storage_folder,
             'module_table_name'=>'product_addons',
   ...
   public function afterCreateProcess($request, $post, $model)
     ...
     public function common_view_data($id)
    {
        ...
        if (count($this->model_relations) > 0) {
            $data['row'] = ProductAddon::with(array_column($this->model_relations, 'name'))->findOrFail($id);
        } else {
            $data['row'] = ProductAddon::findOrFail($id);
        }
        $data['view_inputs']=[];
    ...
    [
        ...
        if ($request->ajax()) {
            ...
            $db_query =  ProductAddon::when(!empty($search_val), function ($query) use ($search_val, $search_by) {
            ...
            if (!can('list_product_addons')) {
                return redirect()->back()->withError('Dont have permission to list');
            }
            $query = null;
            if (count($this->model_relations) > 0) {
                $query = ProductAddon::with(array_column($this->model_relations, 'name'));
            } else {
    ...
    public function store(ProductAddonRequest $request)
    {
        ...
        try {
            ...
            */
             ...
             $product_addon = ProductAddon::create($post);
               $this->afterCreateProcess($request,$post,$product_addon);
             \DB::commit();
    ...
    public function edit(Request $request,$id)
    {
        ...
        $model = ProductAddon::findOrFail($id);
    ...
    public function update(ProductAddonRequest $request, $id)
    {
         if (!can('edit_product_addons')) {
    ...
    {
                if (!can('delete_product_addons')) {
        return createResponse(false,'Dont have permission to delete');
        }
         \DB::beginTransaction();
        try
        {
            if(ProductAddon::where('id',$id)->exists())
    ...
    public function deleteFile($id)
    {
     ...
     public function exportProductAddon(Request $request,$type){
        if (!can('export_product_addons')) {
            return redirect()->back()->withError('Not allowed to export');
        }
      $meta_info=$this->commonVars()['data'];
      return $this->exportModel('ProductAddon','product_addons',$type,$meta_info);
