...
class BulkDiscountExport  implements FromQuery,WithColumnFormatting, WithMapping,WithHeadings
{
    use Exportable;
    /**
   ...
   public function query()
    {
        ...
        if (count($this->model_relations) > 0) {
            $query = BulkDiscount::with(array_column($this->model_relations, 'name'));
        } else {
    ...
    public function map($row): array
    {
        ...
        foreach ($this->export_columns as $colname) {
            $val = null;
            if (isFieldPresentInTableAsColumn('App\Models\BulkDiscount', $colname)) {
                $val = $row->{$colname};
