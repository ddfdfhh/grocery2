    ...
    if (count($product_coupons->toArray()) > 0) {
        foreach ($product_coupons as $coupon_row) {
            ...
            if ($is_eligible_coupon) {
                ...
                if ($minimum_cart_amount_required > 0) {
...
function getProductQtyRules($cart_item, $user_id, $discount_method)
{
    $discount_rules = [];
    $product_id = $cart_item->product_id;
    $item_category_id = \DB::table('products')->whereId($product_id)->first()->category_id;
    $product_rules = \DB::table('coupons')->whereStatus('Active')->whereDate('start_date', '<=', Carbon::now())
        ->whereDate('end_date', '>=', Carbon::now())
        ->whereType('Individual Quantity')->whereDiscountMethod($discount_method)
        ->get();
...
function getCustomerGroups($user_id)
{
    $customer_grp_ids = [];
...
function getBogoOfferForSingleCoupon($coupon_row, $cart_session_id, $user_id, $discount_method)
{
    $offer_product_to_claim = [];
    $on_buy_qty = 0;
    $multiple = 1;
...
function getCartAmountTotalConditionally($include_or_exclude, $cart_items, $coupon_category_ids, $coupon_product_ids)
{
    $sum = 0;
    ...
    if ($loop_only_selected_items) {
        foreach ($cart_items as $item) {
            ...
            ? in_array($p_id, $coupon_product_ids) || (empty($coupon_product_ids) && in_array($cart_item_row->category_id, $coupon_category_ids))
            : (!in_array($p_id, $coupon_product_ids) || (empty($coupon_product_ids) && !in_array($cart_item_row->category_id, $coupon_category_ids)));
            if ($should_proceed) {
...
function shouldCartItemIncludeForCoupon($include_or_exclude, $cart_item, $coupon_category_ids, $coupon_product_ids)
{
    ...
    if (empty($coupon_product_ids) && empty($coupon_category_ids)) {
        ...
        ? in_array($cart_item_row->category_id, $coupon_category_ids)
        : !in_array($cart_item_row->category_id, $coupon_category_ids);

    } elseif (!empty($coupon_product_ids) && !empty($coupon_category_ids)) {
        $should_include = $include_or_exclude == 'Include'
        ? in_array($cart_item_row->product_id, $coupon_product_ids)
        : !in_array($cart_item_row->product_id, $coupon_product_ids);
