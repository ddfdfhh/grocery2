...
function SendSms($toNumber, $otp,$is_live)
{
    $senderId = null;
    $userId = null;
    $templateId = null;
    ...
    if($is_live=='Yes'){
     $senderId = 'OMGHUB';
    $userId = 'OMGHUB';
    $templateId = '1707170262352091957';
    $entityId = '1701170262069220609';
    $msg = $otp . '%20is%20your%20One%20Time%20Password(OTP)%20to%20login.Don%27t%20share%20OTP%20with%20any%20one%20.-%20OMG%20HUB(Online%20Mart%20And%20Gift%20Hub)';
    $key='e3ed681724XX';
    }
    else{
        $key='18c70a7d7bXX';
    ...
    $msg = 'Dear Customer, Your OTP is '.$otp.' Please do not share this OTP. Regards';
...
//    // dd($path);
//     chmod(Storage::path('public/' . $folder.'/'.$fileNameToStore),0755);
...
function storeSingleFileCustomDimension($dimension,$folder, $filerequest, $generateThumbnail = false)
{
    $folder = str_replace('\\', '/', $folder);
    $filenameWithExt = $filerequest->getClientOriginalName();
    $filename = pathinfo($filenameWithExt, PATHINFO_FILENAME);
...
//     chmod(Storage::path('public/' . $folder.'/'.$fileNameToStore),0755);
    ...
    // ImageOptimizer::optimize(storage_path('app/public/' . $folder . '/' . $fileNameToStore));
    if ($generateThumbnail) {
        generateThumbnail($filerequest, $folder, $dimension, $filename, $extension);
    }
    return $fileNameToStore;
}
...
function generateThumbnail(
 ...
 //small thumbnail name
   ...
   // dd(storage_path('app\\' . $folder . '\\thumbnail'));
    if (!File::isDirectory(storage_path('app\\public\\' . $folder . '\\thumbnail'))) {
        File::makeDirectory(storage_path('app\\public\\' . $folder . '\\thumbnail'), 0755, true, true);
        ...
        foreach ($valid_coupons as $coupon_row) {
            ...
            if ($is_eligible_coupon['is_eligible']) {
                ...
                foreach ($cart_items as $cart_item) {
                    $is_item_eligible =
                        shouldCartItemIncludeForCoupon($coupon_row, $cart_item, $coupon_category_ids, $coupon_product_ids);
                    if ($is_item_eligible) {
                        $minimum_cart_amount_required = $coupon_row->minimum_order_amount;
...
function applyAppliedCouponsTableToCart($cart_items, $cart_session_id, $user_id, $should_update_cart = true)
{
    ...
    $applied_coupons = \DB::table('applied_coupons')->select('applied_coupons.*',
        ...
        'coupons.discount_method', 'coupons.discount_type', 'coupons.discount', 'coupons.start_date', 'coupons.end_date')
        ->join('coupons', 'coupons.id', '=', 'applied_coupons.coupon_id')->where(['cart_session_id' => $cart_session_id,
